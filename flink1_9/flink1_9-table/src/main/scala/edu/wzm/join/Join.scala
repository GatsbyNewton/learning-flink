...
import org.apache.flink.table.api.{EnvironmentSettings, TableConfig, Tumble}
...
object Join {
    ...
    def main(args: Array[String]): Unit = {
        ...
        tConfig.setIdleStateRetentionTime(Time.minutes(12), Time.minutes(24))
        tConfig.getConfiguration.setString("table.exec.emit.early-fire.enabled", "true")
        tConfig.getConfiguration.setString("table.exec.emit.early-fire.delay", "1s")
        tConfig.getConfiguration.setString("table.exec.emit.late-fire.enabled", "true")
        tConfig.getConfiguration.setString("table.exec.emit.late-fire.delay", "2s")

        val userDS = env.socketTextStream("localhost", port)
            .assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor[String](org.apache.flink.streaming.api.windowing.time.Time.seconds(30)) {
